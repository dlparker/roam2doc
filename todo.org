...
8. [ ] Allow user to supply css and javascript files on command line, to be merged into <head> by reference
9. [X] Create io.py to manage input and output
10. [ ] Remember to add --enable-local-file-access to wkhtmltopdf invocation, and add it to cli.py
